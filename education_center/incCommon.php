<?php
	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(   
			'courses' => array('Courses', 'Listing of courses actually held or to be held.', 'resources/table_icons/document_comment_above.png', 'None'),
			'enrollment' => array('Student Enrollments', 'Listing of students who attended or will attend a specific course. This also tracks their course score and you can print the course certificate from here.', 'resources/table_icons/data_chooser.png', 'None'),
			'attendance' => array('Attendance', 'Record/check attendance of students here.', 'resources/table_icons/attributes_display.png', 'None'),
			'students' => array('Students Data', 'List of students registered.', 'resources/table_icons/angel.png', 'None'),
			'instructors' => array('Instructors Data', 'List of instructors registered.', 'resources/table_icons/administrator.png', 'None'),
			'labs' => array('Labs', 'List of labs.', 'resources/table_icons/color_swatch.png', 'None'),
			'courses_catalog' => array('Courses catalog', 'List of courses taught here.', 'resources/table_icons/application_from_storage.png', 'None'),
			'companies' => array('Companies', '', 'resources/table_icons/brick.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'courses' => "`courses`.`course_id` as 'course_id', IF(    CHAR_LENGTH(`courses_catalog1`.`course_name`), CONCAT_WS('',   `courses_catalog1`.`course_name`, '  '), '') as 'course_name', IF(    CHAR_LENGTH(`courses_catalog1`.`course_summary`), CONCAT_WS('',   `courses_catalog1`.`course_summary`), '') as 'description', IF(    CHAR_LENGTH(`instructors1`.`inst_name`), CONCAT_WS('',   `instructors1`.`inst_name`), '') as 'instructor_id', IF(    CHAR_LENGTH(`labs1`.`lab_code`), CONCAT_WS('',   `labs1`.`lab_code`), '') as 'lab_id', if(`courses`.`start_date`,date_format(`courses`.`start_date`,'%d.%m.%Y'),'') as 'start_date', if(`courses`.`end_date`,date_format(`courses`.`end_date`,'%d.%m.%Y'),'') as 'end_date', TIME_FORMAT(`courses`.`start_time`, '%r') as 'start_time', TIME_FORMAT(`courses`.`end_time`, '%r') as 'end_time', `courses`.`mon` as 'mon', `courses`.`tue` as 'tue', `courses`.`wed` as 'wed', `courses`.`thu` as 'thu', `courses`.`fri` as 'fri', `courses`.`sat` as 'sat', `courses`.`sun` as 'sun', `courses`.`fees` as 'fees'",
			'enrollment' => "`enrollment`.`rec_id` as 'rec_id', IF(    CHAR_LENGTH(`students1`.`student_name`), CONCAT_WS('',   `students1`.`student_name`), '') as 'stud_id', IF(    CHAR_LENGTH(`courses_catalog1`.`course_name`), CONCAT_WS('',   `courses_catalog1`.`course_name`, '  '), '') as 'course_id', `enrollment`.`score` as 'score', `enrollment`.`notes` as 'notes', `enrollment`.`certificate_notes` as 'certificate_notes'",
			'attendance' => "`attendance`.`attendance_id` as 'attendance_id', IF(    CHAR_LENGTH(`students1`.`student_name`) || CHAR_LENGTH(`courses_catalog1`.`course_name`), CONCAT_WS('',   `students1`.`student_name`, ' / ', `courses_catalog1`.`course_name`, '  '), '') as 'student_course', if(`attendance`.`date`,date_format(`attendance`.`date`,'%d.%m.%Y'),'') as 'date', `attendance`.`attended` as 'attended', `attendance`.`notes` as 'notes'",
			'students' => "`students`.`student_id` as 'student_id', `students`.`student_name` as 'student_name', IF(    CHAR_LENGTH(`companies1`.`company`), CONCAT_WS('',   `companies1`.`company`), '') as 'company', `students`.`email` as 'email', `students`.`phone` as 'phone', if(`students`.`reg_date`,date_format(`students`.`reg_date`,'%d.%m.%Y'),'') as 'reg_date', `students`.`photo` as 'photo', `students`.`notes` as 'notes'",
			'instructors' => "`instructors`.`inst_id` as 'inst_id', `instructors`.`inst_name` as 'inst_name', `instructors`.`email` as 'email', `instructors`.`phone` as 'phone', `instructors`.`fulltime` as 'fulltime', `instructors`.`photo` as 'photo', `instructors`.`notes` as 'notes'",
			'labs' => "`labs`.`lab_id` as 'lab_id', `labs`.`lab_code` as 'lab_code', `labs`.`capacity` as 'capacity', `labs`.`notes` as 'notes'",
			'courses_catalog' => "`courses_catalog`.`cat_id` as 'cat_id', `courses_catalog`.`course_code` as 'course_code', `courses_catalog`.`course_name` as 'course_name', `courses_catalog`.`course_summary` as 'course_summary', `courses_catalog`.`course_contents` as 'course_contents', `courses_catalog`.`notes` as 'notes'",
			'companies' => "`companies`.`company_id` as 'company_id', `companies`.`company` as 'company', `companies`.`notes` as 'notes'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'courses' => "`courses` LEFT JOIN `courses_catalog` as courses_catalog1 ON `courses_catalog1`.`cat_id`=`courses`.`course_name` LEFT JOIN `instructors` as instructors1 ON `instructors1`.`inst_id`=`courses`.`instructor_id` LEFT JOIN `labs` as labs1 ON `labs1`.`lab_id`=`courses`.`lab_id` ",
			'enrollment' => "`enrollment` LEFT JOIN `students` as students1 ON `students1`.`student_id`=`enrollment`.`stud_id` LEFT JOIN `courses` as courses1 ON `courses1`.`course_id`=`enrollment`.`course_id` LEFT JOIN `courses_catalog` as courses_catalog1 ON `courses_catalog1`.`cat_id`=`courses1`.`course_name` ",
			'attendance' => "`attendance` LEFT JOIN `enrollment` as enrollment1 ON `enrollment1`.`rec_id`=`attendance`.`student_course` LEFT JOIN `students` as students1 ON `students1`.`student_id`=`enrollment1`.`stud_id` LEFT JOIN `courses` as courses1 ON `courses1`.`course_id`=`enrollment1`.`course_id` LEFT JOIN `courses_catalog` as courses_catalog1 ON `courses_catalog1`.`cat_id`=`courses1`.`course_name` ",
			'students' => "`students` LEFT JOIN `companies` as companies1 ON `companies1`.`company_id`=`students`.`company` ",
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('education_center_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('education_center_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['education_center_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['education_center_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> education center</a>
			</div>
